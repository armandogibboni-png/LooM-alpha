...
import { CONTEXT_CONFIG, LORE_BY_CONTEXT, MISSION_TEMPLATES, AMBIENT_INTERCEPTS, FALLBACK_TURNS } from './prose.js';
...
export class SwarmEngine {
  constructor(contextKey = 'LOOM_CORE') {
  ...
  // ── AMBIENT INTERCEPTS ───────────────────────────────────────────
  getAmbientIntercept(roomLabel) {
    const pool = AMBIENT_INTERCEPTS?.LOOM_CORE?.[roomLabel] || [];
    if (!pool.length) return null;
    const agents = this.getRoomAgents(roomLabel);
    let text = pool[Math.floor(Math.random() * pool.length)];
    if (agents.length >= 2) {
      text = text.replace('{a}', agents[0].name.split(' ')[0]);
      text = text.replace('{b}', agents[1].name.split(' ')[0]);
    }
    return text;
  }

  getFallbackNarrative() {
    return FALLBACK_TURNS[Math.floor(Math.random() * FALLBACK_TURNS.length)];
  }
